...
- appcontainern serverar Driftpunkt från `/var/www/html/htdocs`
...
Om du inte använder den medföljande Apache-konfigurationen ska webbservern peka mot `htdocs/`, inte `public/`. Om NAS/webbhotell tvingar hela appen att ligga direkt i en yttre `/htdocs`-katalog behöver root-`.htaccess` vara aktiv så att känsliga appmappar blockeras.
...
`app:install:fresh` säkerställer alltid standardkonton för superadmin och admin. Tekniker- och kundkonton skapas om du inte anger `--skip-test-accounts`:
...
- rättar ägare på `htdocs/assets` till `www-data` så brandinguppladdning fungerar
